/*
   load_edgelist from file and convert to uwudgraph.
*/
    ...
    if (num_tokens_found == 0) {
      while ((split_pos = line.find(delimiter)) != std::string::npos) {
        ...
        if (num_tokens_found == 1){
          throw std::invalid_argument("Weight is not supported in uwudgraph");
        }
  ...
  while (std::getline(is, line)) {
    if (line[0] != '#' && line[0] != '/' && line.length() > 0) {
      try {
        ...
        graph->adj[this_edge.second].emplace_back(this_edge.first);
      } catch (std::invalid_argument &e) {
        throw(std::runtime_error(e.what()));
      }
    }
